...
CAutoSizeCombo::CAutoSizeCombo()
{
	maxlen = 0; 
}

BEGIN_MESSAGE_MAP(CAutoSizeCombo, CComboBox)
	ON_CONTROL_REFLECT(CBN_DROPDOWN, OnDropdown)
END_MESSAGE_MAP()

/* CAutoSizeCombm::OnDropdown
 *
 * Result: void
 *       
 * Effect: Sizes the combo to a minimum size required to show everything
 * Notes:
...
void CAutoSizeCombo::OnDropdown() 
{
	int n = GetCount();
	n = max(n, 2);
	...
	{ /* screen limit */
		...
		{ /* invoke limit */
			...
			/* compute new space. Note that we don't really fill
			 * the screen. We only have to use this size if it
			 * is smaller than the max size actually required */
			int ht2 = ht * k;
			sz.cy = min(ht2, sz.cy);
		} /* invoke limit */
	} /* screen limit */
...
BEGIN_MESSAGE_MAP(CIDCombo, CAutoSizeCombo)
END_MESSAGE_MAP()
...
/****************************************************************************
*                        CIDCombo::SelectString
* Inputs:
*       _TCHAR *: ItemData value
* Result: int
*       The index of the selected value, or CB_ERR if no selection possible
* Effect: 
*       If the value exists in the list, selects it, otherwise selects -1
...
int CIDCombo::SelectString(_TCHAR *string)
{
	if (string)
	{
		for(int i = 0; i < GetCount(); i++)
		{ 
			_TCHAR *data = (_TCHAR *)GetItemData(i);

			if(_tcscmp(data, string) == 0)
			{ 
				SetCurSel(i);
				return i;
			} 
...
int CIDCombo::AddStrings(IDStringData * data, int def)
    {
		return AddStrings((IDData *)data, def);     
    }
...
DWORD_PTR CIDCombo::GetCurItem()
    {
     int i = GetCurSel();
     if(i == CB_ERR)
        return 0;
     return (DWORD_PTR)GetItemData(i);
    }

/****************************************************************************
*                         CIDCombo::GetCurItemString
* Result: char *
*       The itemdata of the current selection, or NULL if there is no 
*       selection
****************************************************************************/

_TCHAR *CIDCombo::GetCurItemString()
    {
     int i = GetCurSel();
     if(i == CB_ERR)
        return NULL;
     return (_TCHAR *)GetItemDataPtr(i);
    }
