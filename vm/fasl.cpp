...
namespace scan {
  ...
  static void fast_read_symbol(LRef port, LRef *retval)
  {
    ...
    if (!(PACKAGEP(home) || NULLP(home) || FALSEP(home)))
      fast_read_error("a symbol must either have a package or NIL/#f for home",
                      port, home);
  ...
  static void fast_read_instance_map(LRef port, LRef *new_instance)
  {
    ...
    if (!(INSTANCEP(proto) || FALSEP(proto) || SYMBOLP(proto)))
      fast_read_error("Bad prototype instance, must be #f, a symbol, or an instance", port, proto);
  ...
  static void fasl_ensure_valid_table_index(LRef port, fixnum_t index)
  {
    if (NULLP(PORT_PINFO(port)->_fasl_table)) {
      PORT_PINFO(port)->_fasl_table =
        vectorcons((index >= DEFAULT_FASL_TABLE_SIZE) ? index +  DEFAULT_FASL_TABLE_SIZE :  DEFAULT_FASL_TABLE_SIZE, NIL);
    } else {
      ...
      if (index  >= old_len) {
        ...
        PORT_PINFO(port)->_fasl_table =
          vector_resize(PORT_PINFO(port)->_fasl_table,
                        new_len > SIZE_MAX ? SIZE_MAX : (size_t)new_len,
                        NIL);
      }
    }
        ...
        if (DEBUG_FLAG(DF_FASL_SHOW_OPCODES))
          {
            ...
            dscwritef(DF_FASL_SHOW_OPCODES,
                      ...
                      get_c_long(opcode_location),
                      opcode_name ? opcode_name : _T("<INVALID>"));
          }

        switch(opcode)
          {
          case FASL_OP_NIL:           *retval = NIL;                          break;
          case FASL_OP_TRUE:          *retval = boolcons(true);               break;
          case FASL_OP_FALSE:         *retval = boolcons(false);              break;

          case FASL_OP_CHARACTER:     fast_read_character(port, retval);      break;

          case FASL_OP_LIST:          fast_read_list(port, false, retval);    break;
          case FASL_OP_LISTD:         fast_read_list(port, true, retval);     break;

          case FASL_OP_FIX8:          fast_read_integer(port, 1, retval);     break;
          case FASL_OP_FIX16:         fast_read_integer(port, 2, retval);     break;
          case FASL_OP_FIX32:         fast_read_integer(port, 4, retval);     break;
          case FASL_OP_FIX64:         fast_read_integer(port, 8, retval);     break;

          case FASL_OP_FLOAT:         fast_read_float(port, false, retval);   break;
          case FASL_OP_COMPLEX:       fast_read_float(port, true, retval);    break;

          case FASL_OP_STRING:        fast_read_string(port, retval);         break;
          case FASL_OP_PACKAGE:       fast_read_package(port, retval);        break;
          case FASL_OP_VECTOR:        fast_read_vector(port, retval);         break;

          case FASL_OP_INSTANCE:      fast_read_instance(port, retval);       break;
          case FASL_OP_HASH:          fast_read_hash(port, retval);           break;

          case FASL_OP_CLOSURE:       fast_read_closure(port, retval);        break;
          case FASL_OP_MACRO:         fast_read_macro(port, retval);          break;
          case FASL_OP_SYMBOL:        fast_read_symbol(port, retval);         break;
          case FASL_OP_SUBR:          fast_read_subr(port, retval);           break;
          case FASL_OP_STRUCTURE:     fast_read_structure(port, retval);      break;
          case FASL_OP_STRUCTURE_LAYOUT:fast_read_structure_layout(port, retval);break;
          ...
          case FASL_OP_INSTANCE_MAP: fast_read_instance_map(port, retval);  break;
          ...
          case FASL_OP_BEGIN_LOAD_UNIT:
            if (!allow_loader_ops)
              fast_read_error(_T("load units are not allowed outside loader"), port, lport_location(port));
          ...
          case FASL_OP_END_LOAD_UNIT:
            ...
            if (!PACKAGEP(pinfo->_fasl_stack[pinfo->_fasl_stack_ptr]))
              fast_read_error(_T("Non-package on package stack."), port, lport_location(port));
    ...
    ON_UNWIND()
      {
        ...
        dscwritef(DF_SHOW_FAST_LOAD_FORMS,
                  _T("; DEBUG: done FASL from port: ~a\n"), port);
        dscwritef(DF_SHOW_FAST_LOAD_FORMS,
                  _T("; DEBUG: FASL result: ~s\n"), result);
