...
namespace scan {
                              ...
                              void *user_data)
  {
    ...
    if (binary)
      ...
      {
        SET_PORT_TEXT_INFO(s, (port_text_translation_info_t *)safe_malloc(sizeof(port_text_translation_info_t)));
  ...
  LRef lport_mode(LRef obj)
  {
    ...
    switch(PORT_MODE(obj) & PORT_DIRECTION)
      {
      case PORT_CLOSED:         return keyword_intern(_T("closed"));
      case PORT_INPUT:          return keyword_intern(_T("input"));
      case PORT_OUTPUT:         return keyword_intern(_T("output"));
      case PORT_INPUT_OUTPUT:   return keyword_intern(_T("input/output"));
      }
  ...
  LRef lflush_port(LRef port)
  {
    ...
    if (   (!PORT_BINARYP(port))
           && (PORT_TEXT_INFO(port)->_crlf_translate)
           && (PORT_TEXT_INFO(port)->_needs_lf))
      {
        write_char(_T('\n'), port);
      }
        ...
        for(size_t next_char_to_write = 0; next_char_to_write < count;)
          {
            ...
            for(next_eoln_char = next_char_to_write;
                (next_eoln_char < count);
                next_eoln_char++)
              {
            ...
            if (PORT_TEXT_INFO(port)->_needs_lf)
            ...
            else
              {
                ...
                write_raw(&(buf[next_char_to_write]), sizeof(_TCHAR), next_eoln_char - next_char_to_write, port);
              }
...
#define PORT_FILE(obj) ((FILE *)(PORT_PINFO(obj)->_user_data))
  ...
  LRef lopen_input_file(LRef filename, LRef mode)
  {
    ...
    return fileportcons(&file_port_class, (port_mode_t)(PORT_INPUT | (binary ? PORT_BINARY : 0)), filename);
  }
...
#define DEBUG_PORT_BLOCK_SIZE (256)
  ...
  size_t debug_port_write(const void *buf, size_t size, size_t count, LRef obj)
  {
    ...
    while(len > 0)
      {
        for(block_loc = 0;
            (block_loc < DEBUG_PORT_BLOCK_SIZE) && (len > 0);
...
#define PORT_STRING(obj) (PORT_PINFO(obj)->_user_object)
  ...
  LRef open_c_data_input(bool binary_data, unsigned char *source, size_t bytes)
  {
    ...
    return portcons(&c_data_port_class, NIL, (port_mode_t)(PORT_INPUT | (binary_data ? PORT_BINARY : 0)), NIL, ps);
  }
    ...
    while(bytes_to_read)
      {
        ...
        if (bytes_available)
          {
            memcpy(&((u8 *)buf)[bytes_read], &((u8 *)ps->_buffer)[ps->_buffer_pos], bytes_available);
  ...
  LRef blocking_input_cons(const _TCHAR *port_name, bool binary,
                           ...
                           void *userdata)
  {
    blocking_input_port_state *ps = (blocking_input_port_state *)safe_malloc(sizeof(blocking_input_port_state));
        ...
        switch (ch)
          ...
          case 'u':
            ...
            if (DEBUG_FLAG(DF_PRINT_FOR_DIFF))
              scwritef("#<~a@(no-addr)", port,
                       lrepresentation_of(unprintable_object));
            else
              scwritef("#<~a@~c&", port,
                       lrepresentation_of(unprintable_object),
                       unprintable_object);
          ...
          case 'c': /*  C object prefix */
            ...
            switch (ch)
              ...
              default: vmerror(_T("Invalid C object format character in scwritef"), charcons(ch));
                break;
              };
            break;

          default: vmerror(_T("Invalid format character in scwritef"), charcons(ch));
            break;
          }
