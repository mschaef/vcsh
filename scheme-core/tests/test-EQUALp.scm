...
(define-proto EQUAL?-test-null-instance)

(define-proto EQUAL?-test-base-instance 'x 0 'y 0)
(define-proto EQUAL?-test-base-instance/2 'x 0 'y 0)
(define-proto (EQUAL?-test-derived-instance  EQUAL?-test-base-instance) 'x 0)

(define-test EQUAL?-simple-instance
  (let ((inull (make-instance 'EQUAL?-test-null-instance)))
    (test-case (not (EQUAL? inull :not-inull)))
    (test-case (EQUAL? inull inull))
    (test-case (EQUAL? inull (make-instance 'EQUAL?-test-null-instance)))
    (test-case (not (EQUAL? inull (make-instance 'EQUAL?-test-base-instance)))))

  (let ((i1 (make-instance 'EQUAL?-test-base-instance))
        (i2 (make-instance 'EQUAL?-test-base-instance)))

    (test-case (EQUAL? i1 i1))
    (test-case (EQUAL? i1 i2))
    (test-case (EQUAL? i2 i1))

    (test-case (not (EQUAL? i1 :keyword)))
    (test-case (not (EQUAL? :keyword i1)))
    (test-case (not (EQUAL? i1 "string")))
    (test-case (not (EQUAL? "string" i1)))
    (test-case (not (EQUAL? i1 52)))
    (test-case (not (EQUAL? 42 i1)))
    (test-case (not (EQUAL? i1 (make-instance 'EQUAL?-test-base-instance/2))))
    (test-case (not (EQUAL? (make-instance 'EQUAL?-test-base-instance/2) i1)))

    (let ((value "new-value"))
      (slot-set! i1 'x value)

      (test-case (not (EQUAL? i1 i2)))
      (test-case (not (EQUAL? i2 i1)))
...
(define-test EQUAL?-shared-instance
  (let ((i1 (make-instance 'EQUAL?-test-base-instance))
        (i2 (make-instance 'EQUAL?-test-base-instance))
        (l1 '(1 2 3))
        (l2 '(1 2 3)))
  ...
  (let ((i1 (make-instance 'EQUAL?-test-base-instance))
    ...
    (slot-set! i1 'x (circular-list 1 2 3))
    (slot-set! i2 'x (circular-list 1 2 3))

    (test-case (EQUAL? i1 i2))

    (slot-set! i1 'y (circular-list 1 2 3))
    (slot-set! i2 'y (circular-list 1 2 3))
    (slot-set! i1 'z (circular-list 1 2 3))
    (slot-set! i2 'z (circular-list 1 2 3))

    (test-case (EQUAL? i1 i2)))

  (let ((i1 (make-instance 'EQUAL?-test-base-instance))
        (i2 (make-instance 'EQUAL?-test-base-instance))
        (s3 (make-instance 'EQUAL?-test-base-instance)))
