...
(define *property-hash* (make-hash))

(define (get sym indicator)
  (unless (symbol? sym)
          (error "Expected symbol: ~s" sym))
  (hash-ref *property-hash* (cons sym indicator)))

(define (put sym indicator value)
  (unless (symbol? sym)
          (error "Expected symbol: ~s" sym))
  (hash-set! *property-hash* (cons sym indicator) value))
